(function ($) {
	...
	function loadPostTypes() {
		ajax( 'bsm_get_post_types', {}, function ( data ) {
			...
			// "Forms" isn't a real browsable content type (Step 1 has no use for it), so it's only
			// added to the "fill from post" dropdown, not the page-type picker.
			$fillSel.append( '<option value="wpforms">Forms</option>' );
			$fillSel.val( 'post' );
	...
	/* ---------------- insert a WPForms shortcode into a text/textarea/wysiwyg field ---------------- */

	function formInserterHtml() {
		return '<div class="bsm-form-inserter">' +
			'<button type="button" class="button-link bsm-form-inserter-toggle">+ Insert a form shortcode</button>' +
			'<div class="bsm-form-inserter-box" style="display:none;">' +
			'<input type="text" class="bsm-form-inserter-search" placeholder="Search forms by name…">' +
			'<div class="bsm-form-inserter-results"></div>' +
			'</div></div>';
	}

	$( document ).on( 'click', '.bsm-form-inserter-toggle', function () {
		var $box = $( this ).siblings( '.bsm-form-inserter-box' );
		$box.toggle();
		if ( $box.is( ':visible' ) ) $box.find( '.bsm-form-inserter-search' ).trigger( 'focus' );
	} );

	var searchForms = debounce( function ( $box, term ) {
		var $results = $box.find( '.bsm-form-inserter-results' );
		if ( ! term ) { $results.empty(); return; }
		ajax( 'bsm_search_items', { post_type: 'wpforms', term: term }, function ( data ) {
			if ( ! data.results.length ) { $results.html( '<p class="description">No forms found.</p>'); return; }
			$results.html( data.results.map( function ( r ) {
				return '<button type="button" class="bsm-result-item bsm-form-inserter-pick" data-id="' + r.id + '">' + escapeHtml( r.text ) + '</button>';
			} ).join( '' ) );
		} );
	}, 300 );

	$( document ).on( 'input', '.bsm-form-inserter-search', function () {
		searchForms( $( this ).closest( '.bsm-form-inserter-box' ), $( this ).val() );
	} );

	/** Inserts text at the current cursor position (or replaces the selection) in a text input/textarea. */
	...
	$( document ).on( 'click', '.bsm-form-inserter-pick', function () {
		var id = $( this ).data( 'id' );
		var $row = $( this ).closest( '.bsm-field-row' );
		var $field = $row.find( 'input[data-field], textarea[data-field]' ).first();
		insertAtCursor( $field.get( 0 ), '[wpforms id="' + id + '"]' );

		var $box = $( this ).closest( '.bsm-form-inserter-box' );
		$box.hide().find( '.bsm-form-inserter-results' ).empty();
		$box.find( '.bsm-form-inserter-search' ).val( '' );
	} );
	...
	function bindFillFromPost() {
		updateMappingToggleVisibility();
		$( '#bsm-mapping-box' ).hide();
	...
	/**
	 * "Fill" for a form: there's no title/excerpt/image to map, just one shortcode — so instead of
	 * a server round trip, this sets [wpforms id="X"] directly into any field on the currently open
	 * row whose name or label mentions "form" or "shortcode". Leaves every other field untouched,
	 * same guarantee as the normal post/page fill.
	 */
	function applyFormShortcodeFill( formId ) {
		var shortcode = '[wpforms id="' + formId + '"]';
		var matched = 0;

		state.currentSchema.forEach( function ( sf ) {
			if ( ! sf.supported ) return;
			if ( 'text' !== sf.type && 'textarea' !== sf.type && 'wysiwyg' !== sf.type ) return;
			if ( ! /form|shortcode/i.test( sf.name + ' ' + sf.label ) ) return;

			var $el = $( '[data-field="' + sf.name + '"]' );
			if ( $el.length ) { $el.val( shortcode ); matched++; }
		} );

		if ( ! matched ) {
			alert( 'No field here looks like a form field (its name or label would need to mention "form" or "shortcode"). Use the "+ Insert a form shortcode" link under the specific field instead.' );
		}
	}

	/* ---------------- mapping settings: which source field feeds which "fill from post" role ---------------- */

	var FILL_ROLE_LABELS = { title: 'Title', description: 'Description', image: 'Image', link: 'Link' };

	/** The mapping box only makes sense for post/page-style fills — Forms don't go through title/desc/image/link at all. */
	function updateMappingToggleVisibility() {
		var isForms = 'wpforms' === $( '#bsm-fill-post-type' ).val();
		$( '#bsm-mapping-toggle' ).toggle( ! isForms );
		if ( isForms ) $( '#bsm-mapping-box' ).hide();
	}

	$( document ).on( 'change', '#bsm-fill-post-type', function () {
		updateMappingToggleVisibility();
		if ( $( '#bsm-mapping-box' ).is( ':visible' ) ) loadSourceMapping();
	} );

	$( document ).on( 'click', '#bsm-mapping-toggle', function () {
		var $box = $( '#bsm-mapping-box' );
		$box.toggle();
		if ( $box.is( ':visible' ) ) loadSourceMapping();
	} );

	function loadSourceMapping() {
		var postType = $( '#bsm-fill-post-type' ).val();
		var $area = $( '#bsm-mapping-fields' ).html( '<p class="description">Loading…</p>' );
		$( '#bsm-mapping-result' ).empty();

		ajax( 'bsm_get_source_mapping', { post_type: postType }, function ( data ) {
			var html = '';
			$.each( FILL_ROLE_LABELS, function ( role, label ) {
				var info = data.roles[ role ] || {};
				var autoLabel = info.discovered ? 'Automatic — currently: ' + info.discovered : 'Automatic — currently: WordPress default';
				html += '<label>' + label + '</label>';
				html += '<select data-role="' + role + '">';
				html += '<option value="">(' + escapeHtml( autoLabel ) + ')</option>';
				html += '<option value="__core__"' + ( '__core__' === info.override ? ' selected' : '' ) + '>' + escapeHtml( info.core_label ) + '</option>';
				data.fields.forEach( function ( f ) {
					var selected = info.override === f.name ? ' selected' : '';
					html += '<option value="' + escapeHtml( f.name ) + '"' + selected + '>' + escapeHtml( f.label ) + ' (' + escapeHtml( f.name ) + ')</option>';
				} );
				html += '</select>';
			} );
			$area.html( html );
		}, function ( msg ) {
			$area.html( '<p class="bsm-error">' + escapeHtml( msg ) + '</p>' );
		} );
	}

	$( document ).on( 'click', '#bsm-mapping-save', function () {
		var $btn = $( this ).prop( 'disabled', true ).text( 'Saving…' );
		var postType = $( '#bsm-fill-post-type' ).val();
		var mapping = {};
		$( '#bsm-mapping-fields [data-role]' ).each( function () {
			mapping[ $( this ).data( 'role' ) ] = $( this ).val();
		} );

		ajax( 'bsm_save_source_mapping', { post_type: postType, mapping: mapping }, function ( data ) {
			$( '#bsm-mapping-result' ).html( '<p class="bsm-success">' + escapeHtml( data.message ) + '</p>' );
			$btn.prop( 'disabled', false ).text( 'Save Mapping' );
		}, function ( msg ) {
			$( '#bsm-mapping-result' ).html( '<p class="bsm-error">' + escapeHtml( msg ) + '</p>' );
			$btn.prop( 'disabled', false ).text( 'Save Mapping' );
		} );
	} );
